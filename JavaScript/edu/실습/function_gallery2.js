/*
    웹브라우저가 해당 파일의 html태그들을 모두 읽어들인 행위를 onload속성으로 등록
    로딩이벤트라 부르는데 로딩했을때 function함수가 호출되게하여 body내부에 작성한 
    모든 html을 선택해서 스타일을 정의하거나 이벤트처리를 하기 위함
    만약 스크립트 선언문을 바디의 마지막부분에 작성한다면 onload를 작성하지 않아도 된다
 */
