...
$(function(){
 ...
 function autoPlay(){//1~4버튼이 순서대로 강제 클릭 이벤트가 발생되는 함수
   ...
   /*
    Auto 동작원리에 대해서  알아보자.
   	버튼은 총 4개로, 1번부터 4번까지 차례대로 일정한 간격으로 강제 클릭 이벤트가 반복되어 발생된다면
   	자동으로 배너가 바뀌게 될것입니다.
   	이때 버튼 <a>의 인덱스 값은 0부터 3까지 입니다.
   	그러므로 일정한 간격으로 인덱스값이 0부터 1씩 증가 되고, 
   	값이 4이상이 되면 다시 0으로 돌려 강제 이벤트를 발생 시킵니다
   */
   //이때 값이 4이상되면 변수 btnName의 값을 0으로 초기화함.
   if(btnNum>=4) btnNum=0;
   
   //-변수 btnNum에 저장된 값을 이용해  버튼<a>를 차례로 선택해 온후 , trigger("이벤트명")함수를 적용하여
   // 강제로 클릭 이벤트를 발생 시킵니다.
